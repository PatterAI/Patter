        ...
        try:
            while True:
                ...
                if len(raw) > _MAX_WS_MESSAGE_BYTES:
                    logger.warning(
                        "Oversized Telnyx WebSocket message dropped (%d bytes)",
                        len(raw),
                    )
                    continue
                data = json.loads(raw)
                ...
                if event_type_telnyx == "start" and not stream_started:
                    ...
                    _input_is_mulaw = (
                        getattr(agent, "provider", "openai_realtime")
                        == "openai_realtime"
                    )
                    ...
                    async def _telnyx_transfer(
                        number, *, client_state: str | None = None
                    ):
                    ...
                    async def _telnyx_send_dtmf(
                        digits: str, delay_ms: int = 300
                    ) -> None:
                    ...
                    if digit:
                        ...
                        if on_transcript:
                            try:
                                ...
                                logger.debug(
                                    "on_transcript DTMF dispatch error: %s", _exc
                                )

                elif event_type_telnyx == "error":
                    logger.warning(
                        "Telnyx stream error: %s", data.get("payload") or data
                    )
                    ...
                    async with _httpx.AsyncClient() as _http:
                        ...
                        if resp.status_code == 200:
                            ...
                            if total_cost is not None:
                                metrics.set_actual_telephony_cost(
                                    abs(float(total_cost))
                                )
                                logger.debug(
