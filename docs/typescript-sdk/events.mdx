...
Patter emits events at key moments during a call. Register callbacks in `serve()` or `connect()` to react to call starts, ends, transcripts, and messages.

## Callback Overview

| Callback | Trigger | Available In |
|----------|---------|--------------|
| `onCallStart` | When a call connects | `serve()`, `connect()` |
| `onCallEnd` | When a call disconnects | `serve()`, `connect()` |
