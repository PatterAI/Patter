---
title: "Configuration"
description: "Constructor options for cloud and local modes."
---

# Configuration

The `Patter` constructor accepts either `PatterOptions` (cloud mode) or `LocalOptions` (local mode).
...
|-----------|------|----------|---------|-------------|
| `apiKey` | `string` | Yes | — | Your Patter API key (`pt_...`). |
| `backendUrl` | `string` | No | `wss://api.getpatter.com` | WebSocket URL for the Patter backend. |
| `restUrl` | `string` | No | `https://api.getpatter.com` | REST API URL for agent management. |
