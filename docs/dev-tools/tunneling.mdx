...
`cloudflared` is a small companion package (a Python/npm wrapper around Cloudflare's binary) that Patter spawns on demand. It is **not bundled by default** — the core `getpatter` package stays lean so builds that don't need a tunnel don't pay for it.

Install it once:

<CodeGroup>
```bash Python (with Patter)
pip install "getpatter[tunnel]"
```

```bash Python (standalone)
...
Patter cannot install it for you at runtime — Python/npm forbid silent installs from inside a running process. If you pass `tunnel=True` / `tunnel: true` without `cloudflared` available, the SDK raises a clear error with the `pip install` / `npm install` command you need.
