...
All callbacks are **async functions**. They are passed as parameters to `serve()` (local mode) or `connect()` (cloud mode).

## Available Callbacks

| Callback | Trigger | Mode |
|----------|---------|------|
| `on_call_start` | A call connects | Local, Cloud |
| `on_call_end` | A call ends | Local, Cloud |
| `on_transcript` | Each utterance is transcribed | Local |
| `on_message` | User message received (pipeline mode) | Local, Cloud |
...
In cloud mode, pass callbacks to `connect()`:
