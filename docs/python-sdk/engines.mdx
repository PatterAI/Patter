...
Each class ships as both a **flat alias** (`from patter import OpenAIRealtime`) and a **namespaced** class (`from patter.engines import openai` → `openai.Realtime()`). They are equivalent.
...
```python
from patter.engines import openai as openai_engine
...
```python
from patter.engines import elevenlabs as elevenlabs_engine
