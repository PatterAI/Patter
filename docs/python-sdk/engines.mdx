...
| `language` | `str` | `"en"` | BCP-47 language code (e.g., `"ja"`, `"es"`, `"fr"`). Seeds the auto-generated fallback system prompt (`"Respond in {language}"`). Has no effect when you supply your own `system_prompt` — write the prompt in the target language and OpenAI Realtime will auto-detect from the audio. |
