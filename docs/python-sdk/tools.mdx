...
Tools let your voice agent perform actions during a conversation -- check a database, call an API, transfer a call, or anything else you can expose via a webhook.
...
| `webhook_url` | `str` | Yes | URL that Patter POSTs to when the AI invokes this tool. |
