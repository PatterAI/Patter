...
<Note>
  **Where the session lives.** Each phone call maps to **one** Hermes session. Patter
  passes a stable per-call id as the OpenAI `user` field (`patter-call-<call_id>`), and
  Hermes keys its session off `user` — so multi-turn memory inside a call works without
  any extra wiring. This is on by default for `HermesLLM`.
</Note>
