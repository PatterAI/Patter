...
This connects to the `npx getpatter-mcp` process from the prerequisites. **Prefer this
transport on an always-on machine**: a long-lived `streamable-http` server persists
across idle gaps, whereas a stdio server that OpenClaw launches per session can be reaped
when idle and re-spawns the embedded Patter server each time.
...
binary from its default HTTP transport to stdio. On an always-on deployment, prefer
Option 1 — OpenClaw may tear down an idle stdio server and restart it per session.
