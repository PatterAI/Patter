"""Phase 3 driver: inject §2 (Feature Tour) cells into all 24 notebooks."""
...
def main() -> None:
    ...
    for num, nb_stem in TOPICS:
        ...
        print(f"  injected py §2 → {nb_stem}.ipynb ({len(cells_py)} cells)")

        inject_section(ts_nb, SECTION_MARKER, cells_ts)
        print(f"  injected ts §2 → {nb_stem}.ipynb ({len(cells_ts)} cells)")
